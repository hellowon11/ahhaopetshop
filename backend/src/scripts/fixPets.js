...
const fixAllPetIds = async () => {
  try {
    ...
    for (const pet of finalPets.filter(p => p.type === 'dog')) {
      const petId = pet.petId.padEnd(6);
      const name = pet.name ? pet.name.padEnd(12) : 'UNKNOWN'.padEnd(12);
      const breed = pet.breed ? pet.breed.padEnd(20) : 'UNKNOWN'.padEnd(20);
      console.log(`ID: ${petId} | Name: ${name} | Breed: ${breed} | For Sale: ${pet.isForSale ? 'Yes' : 'No'}`);
    }
