...
import { User, Appointment, GroomingService, DayCareOption } from '../types';
...
const AdminPortal: React.FC = () => {
  ...
  // Update handleEditClick to convert time to 12-hour format for the edit form
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    // Only include daycare options if hasDaycare is true
    if (editFormData.hasDaycare) {
      updateData.dayCareOptions = editFormData.dayCareOptions;
    } else {
      // Set to undefined to remove daycare options
      updateData.dayCareOptions = undefined;
    }
  ...
  const renderDayCareDetails = (dayCareOptions: any) => {
    ...
    const { type, days, morning, afternoon, evening } = dayCareOptions;
    
    // Only display daycare if the type and days are valid AND at least one time slot is selected
    if (!type || !days || days <= 0 || !(morning || afternoon || evening)) {
      return null;
    }
  ...
  const getPriceDetails = (appointment: Appointment) => {
    ...
    if (appointment.totalPrice) {
      const originalTotal = basePrice + dayCarePrice;
      discount = originalTotal - appointment.totalPrice;
    }
    ...
    return {
      ...
      dayCarePrice,
      discount,
      totalPrice: appointment.totalPrice || basePrice + dayCarePrice
    };
  };

  // Add a component for price details dropdown
  const PriceDetailsDropdown = ({ appointment }: { appointment: Appointment }) => {
          ...
          >
            ...
            <div className="space-y-2">
              ...
              {priceDetails.discount > 0 && (
                <div className="flex justify-between">
                  <span className="text-green-600">Member Discount:</span>
                  <span className="text-green-600">-RM {priceDetails.discount.toFixed(2)}</span>
                </div>
              )}
            ...
            <form onSubmit={handleEditSubmit}>
              ...
              {editFormData.hasDaycare && (
                <div className="mb-4 p-3 border rounded bg-gray-50">
                  <h4 className="font-medium mb-2">Daycare Options</h4>
                    ...
                    <select
                      ...
                      onChange={(e) => setEditFormData({
                        ...
                        dayCareOptions: {
                          ...
                          type: e.target.value as 'daily' | 'longTerm'
                        }
                      })}
                      className="w-full p-2 border rounded"
                    >
